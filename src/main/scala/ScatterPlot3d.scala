...
object ScatterPlot3d extends App {
  ...
  val x2 = data.x().map(_.slice(0, 3))  // Takes first three columns
  val window = plot(x2, data.labels(), Array('*', '+', 'o'), Array(Color.RED, Color.BLUE, Color.CYAN))

  window.canvas.setAxisLabels(attributes.map(_.getName).slice(0, 3): _*)
}
